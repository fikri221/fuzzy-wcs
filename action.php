...
if ($action == "update-rule") {
    ...
    $query = mysqli_query($con, "UPDATE `rule` SET `suhu`='$suhu',`kelembapan`='$kelembapan',`hasil`='$hasil' WHERE `id`='$id'");
    if ($query) {
    ...
    $kelembapan = $_POST['kelembapan'];
    $hasil = $_POST['hasil'];
    $query = mysqli_query($con, "INSERT INTO `rule`(`suhu`, `kelembapan`, `hasil`) VALUES ('$suhu','$kelembapan','$hasil')");
    if ($query) {
...
elseif ($action == "update-data-tanaman") {
    ...
    if ($query_suhu && $query_klbp_udara) {
